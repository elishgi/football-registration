...
function RegularPlayersList({ regulars }: { regulars: RegularPlayer[] }) {
  return (
    <div className="regulars-list">
      <h3>קבועים קיימים ({regulars.length})</h3>
      {regulars.length ? (
        <div className="regulars-editor">
          {regulars.map((r) => (
            <form key={r.id} action={updateRegular} className="player-row">
        ...
        <p>עדיין אין שחקנים קבועים. הוסף שחקן כדי שיופיע כאן וברשימת ההרשמה.</p>
      )}
    </div>
  );
}
